...
namespace BGC.Data.Relational
{
    ...
    internal class ComposersDbContext :
        ...
        IDtoFactory
    {
        private static object[] GetIntermediateDtoKeys<TPrincipalDto, TDependantDto>(TPrincipalDto principal, TDependantDto dependantEntity)
            where TPrincipalDto : RelationdalDtoBase
            where TDependantDto : RelationdalDtoBase
        {
            object[] principalKeys = DtoUtils.GetKeys(principal);
            object[] dependantKeys = DtoUtils.GetKeys(dependantEntity);
        ...
        private Type GetIntermediateDtoImplementation<TPrincipalDto, TDependantDto>()
            where TPrincipalDto : RelationdalDtoBase
            where TDependantDto : RelationdalDtoBase
        {
        ...
        public IIntermediateRelationalDto<TPrincipalDto, TDependantDto> GetIntermediateDto<TPrincipalDto, TDependantDto>(TPrincipalDto principal, TDependantDto dependantEntity)
            ...
            where TDependantDto : RelationdalDtoBase
        {
            DbSet set = Set(GetIntermediateDtoImplementation<TPrincipalDto, TDependantDto>());
            object[] keys = GetIntermediateDtoKeys(principal, dependantEntity);
            var result = (set.Find(keys) ?? set.Create()) as IIntermediateRelationalDto<TPrincipalDto, TDependantDto>;

            if (result.Principal == null) result.Principal = principal;
            if (result.Dependant == null) result.Dependant = dependantEntity;
